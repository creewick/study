...
![img](images\DNS\Процесс определения IP-адреса по домену.png) 

1. Когда пользователь запускает веб-браузер и вводит название домена сайта, его ПК отправляет запрос к DNS-серверу интернет-провайдера для получения IP-адреса, на котором находится домен (1).
2. Если DNS-серверы провайдера не обнаруживают в своем кэше информации о запрашиваемом сайте, то отправляют запрос на корневые DNS-серверы (2).
3. Корневой DNS-сервер ищет в своей базе данных информацию о серверах имен хостинг-провайдера, на которых присутствует этот сайт. Далее, он сообщает их кэширующему DNS-серверу провайдера (3).
4. После того, как кэширующий DNS-сервер интернет-провайдера получает информацию о серверах имен хостинг-провайдера он опрашивает любой из них (4) и, в случае получения положительного результата получения IP-адреса (5), помещает в кэш. Кэширование используется для того, чтобы снизить как нагрузку на интернет-каналы, так и для ускорения получения результата запроса.
5. После этого DNS-сервер провайдера передает IP-адрес браузеру пользователя, совершившему запрос сайта (6).
6. И уже после этого браузер, получив IP-адрес запрашиваемого сайта, переходит на сам сайт (7 и 8).

## Иерархический принцип построения

![1](images\DNS\Иерархический принцип построения.png)
...
![1529137542423](images\DNS\Разделение на зоны и домены.png)
...
## Примеры рукерсивного и нерекурсивного запроса resolver'а, использование кэша, рекурсивная процедура разрешения имен.
...
![1529138461000](C:\Users\Zero.net\Desktop\Studying\Typora\inet\routing\images\DNS\Рекурсивный запрос resolver'а.png)
...
![1529138137547](images\DNS\Нерекурсивный запрос resolver'а.png)

![1529138514303](images\DNS\Использование кэша.png)

![1529138550364](images\DNS\Рекурсивная процедура разрешения имен.png)
...
![1529139780409](images\DNS\Stealth-сервер.png)

* Для доменов 2-го уровня обязательно иметь два DNSD сервера.
...
## Принцип работы DNS NOTIFY, RFC 1996

![1529139427475](images\DNS\Принцип работы DNS NOTIFY.png)

![1529139474049](images\DNS\принцип работы DNS NOTIFY 2.png)
